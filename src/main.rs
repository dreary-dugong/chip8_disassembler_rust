fn main() {
    ...
    if let Err(msg) = read_write_asm_file(input_file, output_file) {
        panic!("{}", msg);
    }
}

fn read_write_asm_file(input_file: String, output_file: String) -> Result<(), &'static str> {
    use std::fs;
...
fn disassemble(assembled_bytes: Vec<u8>) -> String {
    assembled_bytes
        ...
        .map(|chunk| {
            if let [b1, b2] = chunk {
                ...
                panic!("Error parsing file: uneven number of bytes");
            }
        })
